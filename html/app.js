...
function addMessage(sourcename, message, playername) {
  ...
  if (
    lastMessageContent !== `[${sourcename}]: ${message}` ||
    currentTime - lastMessageTime >= delayBetweenMessages
  ) {
    const messageElement = document.createElement("div");
  ...
  if (event.data.action === "display") {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (message.startsWith("/image")) {
        const imageUrl = message.substring(6).trim();
        $.post(
          `https://${GetParentResourceName()}/imagesent`,
          JSON.stringify(imageUrl)
        );
      } else if (message) {
        $.post(
          `https://${GetParentResourceName()}/messagesent`,
...
$(function () {
  setInterval(() => {
    if (messageListener) {
      $.post(
        `https://${GetParentResourceName()}/getonlinestaff`,
        JSON.stringify({})
      );
    }
  }, 60000);
